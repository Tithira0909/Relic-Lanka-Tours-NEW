...
import { MapPin, Clock, Calendar, CheckCircle, ChevronDown, User, ArrowLeft } from 'lucide-react';
...
export const TourDetail: React.FC = () => {
  ...
  // Find tour or use first as fallback for demo safety
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex border-b border-gray-100 mb-8">
              <button 
                ...
                className={`pb-4 px-4 font-medium text-lg relative transition-colors ${
                  activeTab === 'overview' ? 'text-ceylon-700' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                Overview
                {activeTab === 'overview' && <motion.div layoutId="tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-ceylon-700" />}
              </button>
              <button 
                onClick={() => setActiveTab('itinerary')}
                className={`pb-4 px-4 font-medium text-lg relative transition-colors ${
                  activeTab === 'itinerary' ? 'text-ceylon-700' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                Itinerary
                {activeTab === 'itinerary' && <motion.div layoutId="tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-ceylon-700" />}
              </button>
            </div>
              ...
              {activeTab === 'overview' ? (
                <div className="space-y-8">
                  ...
                  <div className="bg-paper p-6 rounded-2xl">
                    <h3 className="font-bold text-primary mb-2">Includes</h3>
