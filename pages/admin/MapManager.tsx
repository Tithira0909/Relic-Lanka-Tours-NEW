...
// ✅ Works in BOTH:
// - Dev: set VITE_API_BASE=http://localhost:3005 (or your backend port)
// - Prod: leave empty => same domain
...
const MapManager: React.FC = () => {
  ...
  const fetchPins = async () => {
    try {
      // ✅ changed (env-aware)
      const res = await api.get('/api/map_pins');
      setPins(res.data);
    } catch (err) {
  ...
  const savePin = async () => {
    ...
    try {
      if (editingPin) {
        // ✅ changed (env-aware)
        await api.put(`/api/map_pins/${editingPin.id}`, editingPin, {
        ...
        // ✅ changed (env-aware)
        await api.post('/api/map_pins', pin, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const deletePin = async (id: string) => {
    ...
    try {
      // ✅ changed (env-aware)
      await api.delete(`/api/map_pins/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchPins();
    } catch (err) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="flex gap-6">
        {/* Map Area */}
        <div
          className="relative w-2/3 border rounded overflow-hidden"
          ref={mapRef}
          onClick={handleMapClick}
          style={{ cursor: (isAdding || editingPin) ? 'crosshair' : 'default' }}
        >
          ...
          {pins.map(pin => (
            <div
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (!isAdding && !editingPin) setEditingPin(pin);
              }}
            />
          ))}
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => editingPin
                      ? setEditingPin({ ...editingPin, name: e.target.value })
                      : setNewPin({ ...newPin, name: e.target.value })
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="file"
                    onChange={e => handleImageUpload(e, !!editingPin)}
